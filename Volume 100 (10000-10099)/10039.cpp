/* @JUDGE_ID: 19899RK 10039 C++ "By Anadan" */ 
// Railroads
// Accepted (3.264 seconds using as much as 1368 kbytes)
...
void getarrival(int mint, int maxt, int src, int dst){
	...
	if (i > maxt) printf("No connection\n\n");
		...
		printf("Departure %.2d%.2d %s\n", depart[dst][i] / 60, depart[dst][i] % 60, names[src]);
		printf("Arrival   %.2d%.2d %s\n\n", i / 60, i % 60, names[dst]);
	}
}

int main(){
	int set, scena, i, j, k, m, time[100], hh, mm, max;
	char str[80], *p;

	scanf("%d\n", &set);
	for (scena = 1; scena <= set; ++scena){
		printf("Scenario %d\n", scena);
		scanf("%d\n", &N);
